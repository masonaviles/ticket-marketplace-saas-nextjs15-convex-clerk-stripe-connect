...
/**
 * Helper function to process the waiting list queue.
 * Extracted so it can be called from multiple mutations.
 */
async function processQueueImpl(ctx: any, { eventId }: { eventId: Id<"events"> }) {
  ...
  const { availableSpots } = await ctx.db
    ...
    .then(async (event: any) => {
      ...
      const activeOffers = await ctx.db
        ...
        .then(
          (entries: any) =>
            entries.filter((e: any) => (e.offerExpiresAt ?? 0) > now).length
        );
...
export const processQueue = mutation({
  ...
  handler: async (ctx, { eventId }) => {
    return await processQueueImpl(ctx, { eventId });
  },
});
...
export const cleanupExpiredOffers = internalMutation({
  ...
  handler: async (ctx) => {
    ...
    for (const [eventId, offers] of Object.entries(grouped)) {
      ...
      await processQueueImpl(ctx, { eventId: eventId as Id<"events"> });
    }
  },
});
